...
class Customer { 
    ...
    function insert_db_record () {
        if ($this->fieldsAllValid ()) { // validate user input
            ...
            $sql = "INSERT INTO $this->tableName (name,email,mobile,password) values(?, ?, ?, ?)";
            $q = $pdo->prepare($sql);
            $q->execute(array($this->name,$this->email,$this->mobile,$this->password));
            Database::disconnect();
            header("Location: $this->tableName.php"); // go back to "list"insert_db_record()
        }
        else {
    ...
    function update_db_record ($id) {
        ...
        if ($this->fieldsAllValid()) {
            ...
            $pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE $this->tableName  set name = ?, email = ?, mobile = ?, password = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($this->name,$this->email,$this->mobile,$this->password,$this->id));
            Database::disconnect();
    ...
    function list_records() {
        ...
        echo "
            ...
            <body>
                <a href='https://github.com/SpencerHD/PhpProject5' target='_blank'>Github Repo</a><br />
                <div class='container'>
                    <p class='row'>
                        <h3>$this->title" . "s" . "</h3>
                    </p>
                    <p>
                        ...
                        <a style='background: gray' href='logout.php?fun=display_create_form' class='btn btn-success'>Logout</a>
                    </p>
                    <div class='row'>
        ...
        echo "
                ...
                Simple File Upload
                <form method='post' action='upload01.php' enctype='multipart/form-data'>
                    <p>File</p>
                <input type='file' 
                    name='Filename'> 
                <p>Description</p>
                ...
                Binary Large OBject (BLOB) Upload
                <form method='post' action='upload02.php' 
            ...
            <a href='../NewScreenFlow.png' target='_blank'>Screen Flow Chart</a><br />
            </body>
        </html>
                    ";  
